...
test_that("findAllOverlap gives what is expected", {
  ...
  all.dataframes <- list(first____second = data.frame(first = 1:2, second = c(1, 1)),
  ...
  expect_equal(findAllOverlap(grL, returnAllMergedCalculated = T), all.dataframes)
})
...
test_that("filterByScore gives what is expected", {
  ...
  all.merges.my.dataframe <- data.frame(first = c(1, 2, NA, 1, 1),
                                        second = c(1, 1, 2, NA, 1),
                                        third = c(1, NA, 2, 1, NA))
  expect_equal(filterByScore(grL, mAll = all.merges.my.dataframe, useNormScore = T),
               my.dataframe)
